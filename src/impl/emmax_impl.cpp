...
/*
    ...
    std::cout << "line 356 opt_vg " << opt_vg << std::endl;
...
*/
    ...
    std::cout << "line 372 opt_vg " << opt_vg << std::endl;
/*
    ...
    for( i=0; i<n; ++i ){
        ...
        for( j=0;j<n;++j ){
            H_sqrt_inv_pre.get_matrix()[j][i] = temp; //this is from the R code of emma
        }
    }
    My_matrix<double> H_sqrt_inv (n, n);
    My_matrix<double> temp(n, n);
    T_matrix(eigen_L.get_eigen_vectors(), temp);
    trmul(temp, H_sqrt_inv_pre, H_sqrt_inv );
    */
    ...
    for( i=0; i<n; ++i ){
        std::cout << "Y_t " << i << " " << Y_t.get_array()[i] << std::endl;
    }
...
Emma_result emma_estimates ( const My_Vector<double> & y, const My_matrix<double> & k, const My_matrix<double> & x,
                             const My_matrix<double> & xs, const Eigen_result & eigen_L, int & ngrids, double & llim, double &ulim, double & eps,
                     const std::string & method, const int & maxiter ){
    //std::cout << y.get_length() << "\t" << k.get_num_column() << "\t" << k.get_num_row() << "\t" << x.get_num_row() << std::endl;
    assert(y.get_length() == k.get_num_column() && y.get_length()==k.get_num_row() && y.get_length()==x.get_num_row() );
...
//    }
    ...
    for ( i=0; i < m; ++i ){
        log_deltas.get_array()[i] = (double(i) / ngrids)*(ulim - llim) + llim;
        deltas.get_array()[i] = pow(10, log_deltas.get_array()[i]);
    }
...
//    std::cout << "opt_ve " << opt_ve << std::endl;
    ...
    My_matrix<double> H_sqrt_inv (n, n);
    for( i=0; i<n; i++ ){
        this_value = 1.0/sqrt(eigen_L.get_eigen_values().get_array()[i]+opt_delta);
        for( j=0; j<n; j++ ){
...
//        std::cout << " h0_rss " << h0_rss << std::endl;
        f_stat = (h0_rss / mahalanobis_rss - 1) * p / xs.get_num_column();
//        std::cout << "line 444 " << h0_rss << " " << mahalanobis_rss << std::endl;
        freedome1 = xs.get_num_column();

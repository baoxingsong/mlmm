...
void _get_UtW_( const My_matrix<double> & U, const My_matrix<double> & W, const int & n, const int & q, My_matrix<double> & UtW){
    ...
    for ( i=0; i<n; ++i ) {
        for (j = 0; j < q; ++j) {
            for ( k=0; k<n; ++k ){
                UtW.get_matrix()[i][j] += (U.get_matrix()[i][k] * W.get_matrix()[k][j]);
            }
        }
    }
}
...
void _get_Uty_( const Eigen_result & eigen_L, const My_Vector<double> & y, const int & n, My_Vector<double> & Uty){
    ...
    for ( i=0; i<n; ++i ){
        Uty.get_array()[i]=0.0;
        for ( j=0; j<n; ++j ){
            Uty.get_array()[i]+=eigen_L.get_eigen_vectors().get_matrix()[i][j]*y.get_array()[j];
        }
    }
}
...
void _get_yTPxy_(const int & n, const My_Vector<double> & y, const My_matrix<double> & UtW, const My_Vector<double> & Uty,
    ...
    for( i=0; i<q; ++i ){ // since the index could not be -1, so here we use k+1 as the third index, attention, this only affect the third index
        for( j=0; j<q; ++j ){
            wpw[i][j][k+1] = 0.0;
            for( l=0; l<n; ++l ) {
                wpw[i][j][k+1] += UtW.get_matrix()[l][i]*UtW.get_matrix()[l][j]/lambda_theta_p_1.get_array()[l];
            }
        }
    }
    for( k=0; k<q; ++k ){
    ...
    for(  k=0; k < q; ++k ){
        ...
        // printf("updating yTPxy \t %d \t %10.20f\t%10.20f\t%10.20f\t%10.20f\n", k, yTPxy, ypw.get_matrix()[k][k+1-1], wpw[k][k][k+1-1], ypw.get_matrix()[k][k+1-1]*ypw.get_matrix()[k][k+1-1]/wpw[k][k][k+1-1]);
    }
}
...
double _get_log_det_H_(const My_Vector<double> & lambda_theta_p_1, const int & n){
    double result = 0.0;
    for( int i=0; i<n; ++i ){
        result += log(lambda_theta_p_1.get_array()[i]);
    }
    return result;
}
...
double get_det_WTH_1W_( const int & n, const int & q, const My_matrix<double> & UtW, double *** wpw, const My_Vector<double> & lambda_theta_p_1){
    double WTH_1W = 0.0;
    int i, j = 0;
    for( i=0; i<n; ++i ){
        WTH_1W += UtW.get_matrix()[i][0]*UtW.get_matrix()[i][0]/lambda_theta_p_1.get_array()[i];
    }
    WTH_1W += wpw[j][j][j+1-1];
    for( j=1; j<q; ++j ){
...
int newton_raphson_ll( double* new_lambda, const double & eps, const int & js, const int & n, const int & q, const Eigen_result & eigen_L,
    ...
    int k=1, l=js;
    double y0, y1, y2, d, u, x0, x1=*new_lambda;
    x0=*new_lambda;
...
int newton_raphson_reml( double* new_lambda, const double & eps, const int & js, const int & n, const Eigen_result & eigen_L,
                         ...
                         const int & q, double *** wpw, double *** wppw, double *** wpppw, const My_matrix<double> & UtW,
    ...
    int k=1,l=js;
    double y0, y1, y2, d, p, x0, x1=*new_lambda, u;
    x0=*new_lambda;
    ...
    double trace_Px_G = _get_trace_Px_G_(x0, p, trace_P);
    double trace_Px_G_Px_G = _get_trace_Px_G_Px_G_(x0, p, trace_P, trace_PP);
...
//    assert(y.get_length() == k.get_num_column() && y.get_length()==k.get_num_row() && y.get_length()==x.get_num_row() );
    ...
    int q = c + cx;
    int p = n - c - cx; // in the gemma paper the cx == 1, so in their function they have n-c-1 in the equation (4)
    int m = ngrids+1;
    ...
    _get_UtW_( eigen_L.get_eigen_vectors(), W, n, q, UtW);
    ...
    if (method.compare("REML") == 0){// put the if as out for loop is good for auto AVX
        for( i=0; i<m; ++i ){
...
//            printf("inter loop log_det_H:%10.20f\tyTPxy:%10.20f\tlls.get_array()[i]:%10.20f\tdlls.get_array()[i]:%10.20f\n", log_det_H, yTPxy, lls.get_array()[i], dlls.get_array()[i]);
//            std::cout << "log_det_H " << log_det_H << " yTPxy " << yTPxy << " lls.get_array()[i] "<< lls.get_array()[i] << " dlls.get_array()[i] " << dlls.get_array()[i] <<std::endl;
        }
    }
...
//        std::cout << "here???" << std::endl;
        ...
        opt_lambda = 0.5 * (lambdas.get_array()[opt_i - 1] + lambdas.get_array()[opt_i]);
        double * new_opt_lambda;
        *new_opt_lambda = opt_lambda;
        if (method.compare("REML") == 0) {
            newton_raphson_reml( new_opt_lambda , eps, maxiter, n, eigen_L, y, q, wpw, wppw, wpppw, UtW, Uty, ypw, yppw, ypppw);
        }else if (method.compare("ML") == 0){
...
//    printf("opt_lambda:%10.20f\tlog_det_H:%10.20f\tyTPxy:%10.20f\n", opt_lambda, log_det_H,yTPxy);
//    std::cout << "opt_lambda "<< opt_lambda << " log_det_H " << log_det_H  << " yTPxy " << yTPxy <<std::endl;
    ...
    //double beta_hat;
    //double v_beta_hat;
    //double dlrt = 2 * log(l1/l0);
    //double fwald = (beta_hat*beta_hat)/v_beta_hat;
    //double p_fwald = sf(fwald, x.get_num_column(), n-c-x.get_num_column());
